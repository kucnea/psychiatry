...
const daum = require('react-daum-postcode');
...
// 필드 검사
export function check_field(fld, msg)
{
...
// 필드 오류 표시
export function error_field(fld, msg)
{
    if (msg != "")
        errmsg += msg + "\n";
...
// 필드를 깨끗하게
export function clear_field(fld)
{
...
// 숫자에 , 를 출력
export function number_format(data)
{
...
// 새 창
export function popup_window(url, winname, opt)
{
...
// 폼메일 창
export function popup_formmail(url)
{
    var opt = 'scrollbars=yes,width=417,height=385,top=10,left=20';
    popup_window(url, "wformmail", opt);
}

// , 를 없앤다.
export function no_comma(data)
{
    var tmp = '';
    var comma = ',';
    var i;
...
// 쿠키 입력
export function set_cookie(name, value, expirehours, domain)
{
    var today = new Date();
...
export function textarea_decrease(id, row)
{
    if (document.getElementById(id).rows - row > 0)
        document.getElementById(id).rows -= row;
}

export function textarea_original(id, row)
{
    document.getElementById(id).rows = row;
}

export function textarea_increase(id, row)
{
    document.getElementById(id).rows += row;
}

// 글숫자 검사
export function check_byte(content, target)
{
    var i = 0;
    var cnt = 0;
    var ch = '';
...
// 브라우저에서 오브젝트의 왼쪽 좌표
export function get_left_pos(obj)
{
    var parentObj = null;
    var clientObj = obj;
...
// 브라우저에서 오브젝트의 상단 좌표
export function get_top_pos(obj)
{
    var parentObj = null;
    var clientObj = obj;
...
export function flash_movie(src, ids, width, height, wmode)
{
    var wh = "";
...
export function obj_movie(src, ids, width, height, autostart)
{
    var wh = "";
    if (parseInt(width) && parseInt(height))
        wh = " width='"+width+"' height='"+height+"' ";
...
export function doc_write(cont)
{
    document.write(cont);
}
    ...
    switch(zip_case) {
        case 1 :    //iframe을 이용하여 페이지에 끼워 넣기
            ...
            if (element_wrap == null) {
                ...
                element_wrap.innerHTML = '<img src="//i1.daumcdn.net/localimg/localimages/07/postcode/320/close.png" id="btnFoldWrap" style="cursor:pointer;position:absolute;right:0px;top:-21px;z-index:1" className="close_daum_juso" alt="접기 버튼">';
        ...
        default :   //iframe을 이용하여 레이어 띄우기
            ...
            if (element_layer == null) {
                ...
                element_layer.innerHTML = '<img src="//i1.daumcdn.net/localimg/localimages/07/postcode/320/close.png" id="btnCloseLayer" style="cursor:pointer;position:absolute;right:-3px;top:-3px;z-index:1" className="close_daum_juso" alt="닫기 버튼">';
 ...
 * 텍스트 리사이즈
**/
export function font_resize(id, rmv_class, add_class)
{
    var $el = $("#"+id);
